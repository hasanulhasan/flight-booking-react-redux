...
import { bookDate, deleteBook } from '../redux/actions';
import './Home.css'
import logo from '../img/lws-logo.svg'

const Home = () => {
  const books = useSelector((state) => state.travelInfo);
  const dispatch = useDispatch();

  const HandleSubmit = (e) => {
    e.preventDefault();
    const form = e.target
    const des_from = form.from.value;
    const des_to = form.to.value;
    const des_date = form.date.value;
    const guests = form.guests.value;
    const ticketClass = form.ticketClass.value;
    const travelInfo = { des_from, des_to, des_date, guests, ticketClass }
    dispatch(bookDate(travelInfo))
  }
        ...
        <div className="mt-[160px] mx-4 md:mt-[160px] relative">
          <div className="bg-white rounded-md max-w-6xl w-full mx-auto">
            <form className="first-hero lws-inputform" onSubmit={HandleSubmit}>
              ...
              <button disabled={books.length === 3 ? true : false}
                className="addCity" type="submit" id="lws-addCity">
        ...
        <div className="table-container">
          <table className="booking-table">
            ...
            <tbody className="divide-y divide-gray-300/20" id="lws-previewBooked">
              {/* <!-- Row 1 --> */}
              {
                books.map((book, i) => <tr key={i} className="lws-bookedTable text-black">
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      <p className="lws-bookedFrom">{book.des_from}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <p className="lws-bookedTo">{book.des_to}</p>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <p className="lws-bookedDate">{book.des_date}</p>
                  </td>
                  <td className="px-6 py-4 text-center">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex justify-center gap-4">
                      <button className="lws-remove" onClick={() => handleDelete(book)}>
